...
export default function NutritionChart({ protein = 0, carbs = 0, fat = 0 }) {
  ...
  return (
    <div className="nutrition-chart">
      ...
      >
        <div
          style={{ width: `${pPct}%`, backgroundColor: "#e74c3c" }}
        ...
        ></div>
        <div
          style={{ width: `${fPct}%`, backgroundColor: "#f1c40f" }}
          title={`Fat: ${fat}g`}
        ></div>
      </div>
      <div
      ...
      >
        <span>
          <b style={{ color: "#e74c3c" }}>■</b> Protein ({protein}g)
        </span>
        <span>
          <b style={{ color: "#3498db" }}>■</b> Carbs ({carbs}g)
        </span>
        <span>
          <b style={{ color: "#f1c40f" }}>■</b> Fat ({fat}g)
        </span>
      </div>
    </div>
  );
...
export function WeeklyCalorieChart({ data = [] }) {
  // Extract calorie value regardless of whether backend sent 'calories' or 'total_calories'
  const getCal = (d) => Number(d.calories ?? d.total_calories ?? 0);
  const maxCalories = Math.max(...data.map(getCal), 2000);
  ...
  return (
    ...
    >
      {data.map((day, i) => {
        const cal = getCal(day);
        const heightPct = Math.max(
          Math.min((cal / maxCalories) * 100, 100),
          cal > 0 ? 8 : 3,
        );
    ...
    >
      {data.slice(-7).map((day, i) => {
        ...
        return (
          ...
          >
            <span style={{ fontSize: "11px", width: "45px", color: "#888" }}>
